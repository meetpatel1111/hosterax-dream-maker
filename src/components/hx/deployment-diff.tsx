...
export function DeploymentDiffModal({ deployments, initialBaseId, onClose }: Props) {
  const [baseId, setBaseId] = useState<string>(
    initialBaseId ?? deployments[1]?.id ?? deployments[0]?.id ?? "",
  );
        ...
        <div className="grid gap-4 border-b border-border bg-surface/50 p-4 md:grid-cols-2">
          <div className="space-y-1">
            ...
            <select
              ...
              className="w-full rounded-md border border-input bg-input/40 px-3 py-2 font-mono text-xs outline-none focus:border-primary"
            >
              {deployments.map((d) => (
                <option key={d.id} value={d.id}>
                  {d.version || d.id.slice(0, 8)} · {d.commit_sha ?? ""}
                </option>
              ))}
            </select>
          </div>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto p-6">
          <div className="grid gap-6 md:grid-cols-2">
            {[base, compare].map((d, i) => (
              <div
                key={i}
                className="space-y-3 rounded-xl border border-border/80 bg-surface/30 p-5"
              >
                <div className="flex items-center justify-between border-b border-border/60 pb-3">
                  <span className="font-mono text-sm font-semibold text-primary">
                    {d?.version || (i === 0 ? "Release A" : "Release B")}
                  </span>
                ...
                <Row
                  k="Duration"
                  v={d?.duration_ms ? `${(d.duration_ms / 1000).toFixed(1)}s` : "—"}
                  mono
                />
              </div>
            ))}
...
function Row({ k, v, mono }: { k: string; v: string; mono?: boolean }) {
  return (
    <div className="flex justify-between border-b border-border/40 py-1 text-xs">
      ...
      <span className={`max-w-[220px] truncate text-foreground ${mono ? "font-mono" : ""}`}>
        {v}
      </span>
    </div>
  );
