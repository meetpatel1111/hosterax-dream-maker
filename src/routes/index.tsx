    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative overflow-hidden grid-bg">
        <div className="mx-auto max-w-6xl px-6 py-24 text-center">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-lg text-muted-foreground">
            Push-to-deploy from any git repo. Auto-detects 42 stacks, provisions SSL, manages databases, streams build logs — zero YAML.
          </p>
          <div className="mt-8 flex justify-center gap-3">
            <Link to="/auth" className="rounded-md bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground glow-primary hover:opacity-90">
              Launch control plane
            </Link>
            <a href="#features" className="rounded-md border border-border px-5 py-2.5 text-sm font-medium hover:bg-accent">
              What's inside
            </a>
          </div>
          <div className="mx-auto mt-14 max-w-3xl rounded-lg border border-border bg-surface p-4 font-mono text-left text-sm text-muted-foreground shadow-2xl">
            ...
            <div><span className="text-primary">$</span> hx deploy github.com/acme/api</div>
            <div>→ detected: Node.js · Express</div>
            <div>→ building image (14s)</div>
            <div>→ SSL: acme-api.hosterax.app <span className="text-success">✓</span></div>
            <div>→ live: <span className="text-primary">https://acme-api.hosterax.app</span></div>
          </div>
        </div>
      </section>
      ...
      <section id="features" className="border-t border-border bg-surface/40">
        <div className="mx-auto max-w-6xl px-6 py-20">
          <h2 className="text-3xl font-semibold tracking-tight">Everything the PaaS gives you.<br />Without the vendor.</h2>
          <div className="mt-12 grid gap-4 md:grid-cols-3">
            {[
              { i: Rocket, t: "Zero-config deploys", d: "42 stacks across 12 languages, auto-detected." },
              { i: Shield, t: "Automatic SSL", d: "Let's Encrypt with wildcard + auto-renewal." },
              { i: Database, t: "Managed backends", d: "Postgres, MySQL, Mongo, Redis. One-click." },
