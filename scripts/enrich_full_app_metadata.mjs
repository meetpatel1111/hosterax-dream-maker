...
async function main() {
  console.log(
    "Fetching selfh.st complete metadata (software, companions, languages, licenses, tags)...",
  );
  const [soft, comp, langs, lics, tags] = await Promise.all([
    fetch("https://raw.githubusercontent.com/selfhst/cdn/main/directory/software.json").then((r) =>
      r.json(),
    ),
    fetch("https://raw.githubusercontent.com/selfhst/cdn/main/directory/companions.json").then(
      (r) => r.json(),
    ),
    fetch("https://raw.githubusercontent.com/selfhst/cdn/main/directory/languages.json").then((r) =>
      r.json(),
    ),
    fetch("https://raw.githubusercontent.com/selfhst/cdn/main/directory/licenses.json").then((r) =>
      r.json(),
    ),
    fetch("https://raw.githubusercontent.com/selfhst/cdn/main/directory/tags.json").then((r) =>
      r.json(),
    ),
  ]);
  ...
  const processRow = (row) => {
    ...
    if (app) {
      ...
      if (website && (!app.website || app.website.includes("self-hosted.net")))
        app.website = website;
  ...
  console.log(
    `Enriched ${enrichedCount} apps with Language, License, Updated Date, and Categories!`,
  );
