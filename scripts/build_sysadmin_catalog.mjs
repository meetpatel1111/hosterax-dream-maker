...
const SYSADMIN_TAGS = [
  ...
  { label: "Build and software organization tools", slug: "build-and-software-organization-tools", icon: "🏗️" },
  ...
  { label: "Configuration Management Database", slug: "configuration-management-database", icon: "🗄️" },
  { label: "Continuous Integration & Continuous Deployment", slug: "continuous-integration--continuous-deployment", icon: "🚀" },
  ...
  { label: "DNS - Control Panels & Domain Management", slug: "dns---control-panels--domain-management", icon: "🌐" },
  ...
  { label: "Identity Management - Single Sign-On (SSO)", slug: "identity-management---single-sign-on-sso", icon: "🔐" },
  { label: "Identity Management - Tools and web interfaces", slug: "identity-management---tools-and-web-interfaces", icon: "🛠️" },
  ...
  { label: "Network Configuration Management", slug: "network-configuration-management", icon: "🌐" },
...
for (let i = 0; i < lines.length; i++) {
  ...
  if (line.startsWith('### ') || line.startsWith('## ')) {
    ...
    if (
      ...
      const match =
        SYSADMIN_TAG_LOOKUP.get(raw.toLowerCase()) ||
    ...
    if (match && currentTagObj) {
      ...
      rest = rest.replace(/`[^`]+`/g, '').replace(/\(\[Source Code\]\([^)]+\)\)/gi, '').trim();
...
fs.writeFileSync('src/lib/awesome-selfhosted-db.json', JSON.stringify(combinedData, null, 2), 'utf8');
fs.writeFileSync('hosterax/engine/src/awesome-selfhosted-db.json', JSON.stringify(combinedData, null, 2), 'utf8');
