...
console.log(`Loaded ${dashboardMap.size} dashboard-icons mappings and ${simpleMap.size} simple-icons mappings.`);
...
for (const app of db.apps) {
  ...
  if (ghMatch && !['github.com/torvalds', 'github.com/awesome-selfhosted', 'github.com/awesome-foss'].includes(ghMatch[0])) {
    const org = ghMatch[1];
  ...
  // Assign optimal logo
  app.logoUrl = candidateSvg || ghAvatar || siteFavicon || `https://cdn.jsdelivr.net/gh/walkxcode/dashboard-icons/svg/${slugLow}.svg`;
