  ...
  try {
    ...
    if (id)
      publish(id, {
        ts: now,
        stream: "system",
        text: `[edge] vhost written → ${confPath} (${host} → :${port})`,
      });
...
function generateZeroConfigDockerfile(workdir) {
  if (fs.existsSync(path.join(workdir, "package.json"))) {
    return `# Zero-config Dockerfile generated by HosteraX
...
EXPOSE 3000
CMD ["sh", "-c", "if [ -d 'build' ]; then serve -s build -l tcp://0.0.0.0:3000; elif [ -d 'dist' ]; then serve -s dist -l tcp://0.0.0.0:3000; else npm start; fi"]
`;
  }
  if (
...
async function startService(deploymentId, project, workdir, cmd, env, target, port) {
  ...
  if (target === "docker") {
    ...
    if (isDirectImage) {
      ...
      // Ensure a Dockerfile exists; if missing, generate a zero-config multi-stage Dockerfile
...
`;
    ...
    let portFlag = `-p 127.0.0.1:${hostPort}:${containerInternalPort}`;
...
const selfHeal = new SelfHealEngine({
  ...
  restartService: async (project) => {
    ...
    const cleanName = project.toLowerCase().replace(/[^a-z0-9]/g, "_");
    if (p.target === "docker") {
      const inspectRes = spawnSync("docker", ["inspect", "--format", "{{.State.Status}}", `hx_${cleanName}`], { timeout: 3000 });
      if (inspectRes.status === 0) {
        spawnSync("docker", ["restart", `hx_${cleanName}`], { timeout: 10000 });
      } else {
        const dep = db
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if ((m = url.pathname.match(/^\/api\/projects\/([^/]+)\/domains$/)) && req.method === "POST") {
      ...
      db.prepare("INSERT INTO domains VALUES (?,?,?,?,?,?,?,?,?)").run(
        id,
        m[1],
        b.hostname,
        0,
