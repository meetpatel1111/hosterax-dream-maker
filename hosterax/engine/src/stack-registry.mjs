...
export const STACK_ORDER = ["nextjs","nuxt","sveltekit","astro","remix","tanstack-start","angular","gatsby","vite","cra","vue","nestjs","adonis","elysia","hono","fastify","koa","express","django","flask","fastapi","gin","fiber","echo","go","actix","axum","rocket","rust","rails","sinatra","laravel","symfony","springboot","quarkus","kotlin","blazor","dotnet","phoenix","python","docker-compose","docker","static","node"];
...
export function detectPackageManager(files) {
  ...
  if ([...files].some((f) => f.endsWith(".csproj") || f.endsWith(".fsproj") || f.endsWith(".sln"))) return "dotnet";
...
export const WORKSPACE_DETECTORS = [
  ...
  { id: "gradle", label: "Gradle multi-project", markers: ["settings.gradle", "settings.gradle.kts"] },
...
function readDeps(dir, files) {
  ...
  if (files.has("go.mod")) for (const l of read("go.mod").split(/\r?\n/)) {
    const t = l.trim().replace(/^require\s+/, "").split(/\s+/)[0];
    if (t && /^[a-z0-9.\-]+\//.test(t)) add(t);
  }
  if (files.has("cargo.toml")) {
    let inDeps = false;
    for (const l of read("Cargo.toml").split(/\r?\n/)) {
      if (/^\[/.test(l.trim())) { inDeps = /dependencies\]/.test(l); continue; }
      if (inDeps) add(l.split("=")[0].trim());
    }
  }
  for (const f of ["requirements.txt", "Pipfile", "pyproject.toml"]) if (files.has(f.toLowerCase()))
    for (const l of read(f).split(/\r?\n/)) add(l.trim().split(/[=<>!~\[ ;#]/)[0].replace(/^"|"$/g, ""));
  if (files.has("gemfile")) for (const l of read("Gemfile").split(/\r?\n/)) {
    const mm = l.match(/gem\s+["']([^"']+)/); if (mm) add(mm[1]);
  }
  ...
  if (files.has("pom.xml")) {
    ...
    for (const mm of x.matchAll(/<groupId>([^<]+)<\/groupId>\s*<artifactId>([^<]+)</g)) { add(mm[1] + ":" + mm[2]); add(mm[2]); }
  }
  for (const f of ["build.gradle", "build.gradle.kts"]) if (files.has(f)) {
    for (const mm of read(f).matchAll(/["']([a-z0-9.\-]+):([a-z0-9.\-]+)/gi)) { add(mm[1] + ":" + mm[2]); add(mm[2]); }
  }
  if (files.has("mix.exs")) for (const mm of read("mix.exs").matchAll(/\{:([a-z_0-9]+)/g)) add(mm[1]);
  for (const f of files) if (f.endsWith(".csproj") || f.endsWith(".fsproj"))
...
function listFiles(dir) {
  ...
  for (const sub of ["config", "bin", "src", "lib", "app"]) {
    try { for (const n of fs.readdirSync(path.join(dir, sub))) out.add((sub + "/" + n).toLowerCase()); } catch {}
  }
  return out;
}
...
export function detectStackDir(dir) {
  ...
  const depHit = (s) => s.deps.some((d) => deps[d] || Object.keys(deps).some((k) => k.startsWith(d)));
  ...
  for (const id of STACK_ORDER) {
    ...
    switch (id) {
      case "vite": fileMatch = markerHit(s) && !hasBackendMarker(); break;
      case "cra": fileMatch = has("package.json"); break;
      case "vue": depMatch = !!deps["vue"] && !deps["nuxt"]; break;
      case "elysia": case "hono": case "fastify": case "koa": case "express": fileMatch = has("package.json"); break;
  ...
  return { id: "unknown", name: "Unknown", stack: "custom", language: "multi", category: "generic", icon: "🔧",
    port: null, build: null, start: null, outputDir: null, rootMarkers: [], deps: [], marker: null,
    packageManager: detectPackageManager(files), install: null, workspace: detectWorkspace(dir, files) };
}
...
export function detectWorkspace(dir, files = listFiles(dir)) {
  for (const w of WORKSPACE_DETECTORS) {
    ...
    if (!hit) continue;
    if (w.content) {
      try { if (!w.content.test(fs.readFileSync(path.join(dir, hit), "utf8"))) continue; } catch { continue; }
    }
    return w;
  }
  return null;
...
export const DETECTORS = STACK_ORDER.flatMap((id) => {
  const s = STACK_REGISTRY[id];
  return s.rootMarkers.map((file) => ({ file, stack: s.name, build: s.build, start: s.start, port: s.port }));
});
